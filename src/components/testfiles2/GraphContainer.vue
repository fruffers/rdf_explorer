...
export default {
  ...
  methods: {
    ...
    dragAlongHandler () {
      // have the edge be dragged along with component
      // need to know which edge it is............ this would be easier if edge was a child of node
      // or we can identify it using fromnode and tonode properties

      // use node.id to identify the node and its edge connections then change the x and y values in the edges array which are bound
      // to the edge which is picked out with v-for
      // var x = 0
      // for (x in this.nodes) {
      //   if (this.nodes[x].id === dragNode.id) {
